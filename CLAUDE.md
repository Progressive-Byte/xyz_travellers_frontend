...
`plans/{admin,front,guest,host}/*.md` are chunked implementation plans that were used to build out each portal incrementally (e.g. `plans/front/HOST_PORTAL_CHUNKED_PLAN.md` is the source of truth for host portal chunk numbering). `tasks/todo.md` logs completed chunks with review notes; `tasks/lessons.md` records project-specific regressions and conventions learned during development (fonts/colors, hero search bar behavior, listing card rules, host workflow contract details) — read it before touching areas it covers, and add to it after any user correction, following its existing structure.
